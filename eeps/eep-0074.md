...
errors/warnings/information messages emitted by various tools within the
Erlang ecosystem, including - but not limited to - the `erlc` Erlang compiler
and the `dialyzer` type checker.

The catalogues are not limited to tools shipped with Erlang/OTP, but it
can include third-party applications such as the [EqWAlizer][]
type-checker or the [Elvis][] code style reviewer.

Each diagnostic in a catalogue is identified by a **code**
...
The namespace for a diagnostic code is not maintained centrally, but
left up to the community to co-ordinate. It is recommended that one
searches online before taking a namespace, just as when taking an
application name.
...
Errors cannot be re-used. If a tool stops emitting a diagnostic code, the
_deprecated_ code is still documented in the index, together
...
* `$CODE` - The number of the diagnostic, for example `0001`.
...
This function will search the `doc/index/` folders of all applications
