...
We also need support for using matchspecs with native-records. Normally
when using matchspecs with tuple-records you do something like this:

```erlang
-record(foo, {a, b, c}).
[{#foo{ a = Value, b = '$1', _ = '_' }, [], ['$1']}].
```

So for this usecase we would need to support the wildcard initialization
operator. If we don't support it, we would just force users to initialize
all fields themselves with `'_'` as all fields needs to have a value.
