...
Each diagnostic in an index is identified by a **code**
and it is accompanied by a description, examples and possible courses
of action. Diagnostic codes are _namespaced_ based on the tool that
generates them.
...
error or information) we relief tools from having to condense a
...
An good example of this is the _expression updates a literal_ warning
...
The code make it possible to link the warning message to an external
...
forums and chats, where the exact message could vary, but the
diagnostic code would be the same.
...
Diagnostic cannot cannot be re-used. If a tool stops emitting a diagnostic code,
the _deprecated_ code is still documented in the index, together
...
will be placed in the `doc/index` folder and the files should follow this format:
...
* `$ALIAS` - an optional human-readable short-hand for the diagnostic, for example
  `update-literal`.
* `$EXTENSION` - Any file extension, though only `.md` will be rendered nicely
  by `erl`.

Additions to Erlang/OTP
-----------------------

To be able to fetch the detailed diagnostic information easily, two new APIs
...
This function will search the `doc/diagnostic-codes/` folders of all applications
...
{ok, [#{application => compiler,
        filename => "/home/erlang/lib/compiler/doc/index/ERL-0001-update-literal.md",
...
documents, while any other document type be printed verbatim.
...
### io and code module extensions

The `io` and `code` modules will be extended with functions that emit
diagnostics to standard_error.

Actually, lets create a module called `erl_diagnostic` where we will place
all these functions and also the additions to application above.

````erlang
-doc """
A diagnostic item.
""".
-nominal code:diagnostic() :: #{
    anno => erl_anno:anno(),
    severity => error | warning | info | unicode:chardata(),
    diagnostic_code => erl_error:code(),
    diagnostic_switch => unicode:chardata(),
    message => unicode:chardata(),
    source => unicode:chardata()
}.

-doc """
Execute `Fun` and capture all diagnostics generated by it. Returns the return value
of `Fun` and a list of diagnostics.

Diagnostics are generated when `io:diagnostic/4` is called. For example:

```erlang
1> code:capture_diagnostics(fun() ->
     Anno = erl_anno:set_filename("test.erl", erl_anno:new(1)),
     io:diagnostic(warning, "Unused variable ~ts", ["A"], Anno,
       #{ diagnostic_code => ~"ERL-0001", diagnostic_switch => ~"+warn_unused_vars",
          source => "erl_lint" }),
     ok
   end).
{ok, [Diagnostic]}
```
""".
-spec code:capture_diagnostics(Fun :: fun(() -> term())) -> {Result :: term(), [diagnostic()]}.

-doc #{ equiv => code:print_diagnostics(standard_error, Diagnostics, #{}) }.
-spec code:print_diagnostics(Diagnostics :: [diagnostic()]) -> ok.
-doc #{ equiv => code:print_diagnostics(standard_error, Diagnostics, Options) }.
-spec code:print_diagnostics(Diagnostics :: [diagnostic()], Options) -> ok.
-doc """
Prints `Diagnostics` to `Device`.
""".
-spec code:print_diagnostics(Device :: io:device(), Diagnostics :: [diagnostic()],
    Options :: #{ explain => io:format(), format => tty | json | text }) -> ok.

-doc """
Print a diagnostic to `t:standard_error/0`.

The diagnostics generated can also be captured by `code:capture_diagnostics/1` and
then later printed by `code:print_diagnostics/3`
""".
-spec io:diagnostic(
        Severity :: error | warning | info | unicode:chardata(),
...
```erlang
> erlc t.erl
t.erl:5:5: Warning: variable 'A' is unused [ERL-0001]
%    5| foo(A) -> ok.
%     |     ^
%  help: call `erlc -explain ERLC-0001` to see a detailed explanation
```

This mimics how rustc prints diagnostics.
...
standardized parseable format. This should be done
...
```yaml
{
  ...
  severity: "warning",
  code: "DIA-1234",
  doc_uri: "https://erlang.org/doc/apps/dialyzer/DIA-1234",
  source: "dialyzer",
